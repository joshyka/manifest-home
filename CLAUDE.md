...
All go through `frontend/src/lib/api.ts`. The `request()` helper auto-injects the Supabase JWT, handles 401 (signs out). Add new endpoints here, typed.
...
- Login methods: Google OAuth (primary) or PIN login (email + numeric password, for local testing)

### Onboarding

- Shown once per **account**, not per device — flag stored as `onboarded BOOLEAN` in the `settings` table
- `App.tsx` reads `onboarded` from the `GET /api/settings` response (already fetched for auth); no extra request
- On completion, calls `POST /api/onboarding/complete` which sets `onboarded = true` via `set_onboarded()` in `data.py`
- `onboarded` is intentionally excluded from `DEFAULT_SETTINGS` and `SettingsBody` so the settings PUT cannot reset it
- Rows that pre-date the column have `NULL` — treated as `true` (existing account, skip onboarding)
...
| Table | Purpose |
| --- | --- |
| `settings` | User's savings plan (prices, contributions, loan info), onboarding status |
...
See `.env.example`. Backend: `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `FRONTEND_URL`, `CRON_SECRET`. Frontend: `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`.
