...
export default function Maps() {
  ...
  function toggleAmenity(query: string) {
    setAmenity(a => a === query ? null : query)
  }
  ...
  if (amenity && dest) {
    ...
    const o = encodeQ(origin)
    const d = encodeQ(dest)
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Get directions and explore nearby amenities</p>
      </div>
      ...
      <div className="card space-y-4">
        ...
        <div className="grid grid-cols-[1fr_auto_1fr] items-end gap-3">
          <AddressInput
            ...
            value={from}
            onChange={setFrom}
            placeholder="e.g. Folkungagatan 45, Stockholm"
          />
          <button
            onClick={swap}
          ...
          <AddressInput
            label="To"
            value={to}
            onChange={v => { setTo(v); setAmenity(null) }}
            placeholder="e.g. Kungsgatan 10, Stockholm"
          />
        </div>
        ...
        <div>
          ...
          {/* Radius selector — only shown when an amenity is active */}
          {amenity && (
          ...
          {amenity && !dest && (
            <p className="text-xs text-gray-400 mt-2">Enter a "To" address to search nearby.</p>
          )}
          {amenity && dest && (
            <p className="text-xs text-gray-400 mt-2">Tap the highlighted filter again to go back to directions.</p>
          )}
        </div>
